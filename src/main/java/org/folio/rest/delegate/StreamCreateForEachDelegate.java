...
@Service
@Scope("prototype")
public class StreamCreateForEachDelegate extends AbstractRuntimeDelegate {
  ...
  public void execute(DelegateExecution execution) throws Exception {
    ...
    if (endpoint != null && target != null && source != null) {
      ...
      streamService.map(primaryStreamId, delegate -> {

        System.out.print(".");

        String returnData = delegate;
        try {

          ObjectNode destNode = (ObjectNode) objectMapper.readTree(delegate);
          JsonNode srcNode = getSourceNode(sourceValue, destNode);
          ArrayNode ids = objectMapper.createArrayNode();

          if (srcNode != null && srcNode.isArray()) {
            srcNode.forEach(node -> {
              if (node.isArray()) {
                node.forEach(child->{
                  runFindOrCreate(DEFAULT_TENANT, token, targetValue, sourceValue, uniqueByValue, endpointValue, destNode, ids, child);
                });
              } else {
                runFindOrCreate(DEFAULT_TENANT, token, targetValue, sourceValue, uniqueByValue, endpointValue, destNode, ids, node);
              }
            });
    ...
    for (int i = 0; i < sourceParts.length; i++) {
      ...
      if (sourceNode.isArray()) {
        ...
        sourceNode.forEach(node->{
          workingArray.add(node.get(part));
        });
